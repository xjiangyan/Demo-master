...
public class FinalHttp {
    ...
    private List<HttpHandler> tasks = new ArrayList<HttpHandler>();
    ...
    public boolean configCookieStore() {
        if (cookieStore != null) {
    ...
    public void getCookie(Map<String, String> map) {
        if (cookieStore != null) {
  ...
  //------------------put 请求-----------------------
    ...
    public void cancelAllTasks() {
        Log.d("Steven", "try cancelAllTasks");
        for (int i = 0; i < tasks.size(); i++) {
            HttpHandler task = tasks.get(i);
            if (task != null && task.getStatus() != AsyncTask.Status.FINISHED
                    && !task.isCancelled()) {
                Log.d("Steven", "task "+task.toString()+" start cancel");
                task.cancel(true);
                tasks.remove(task);
                task = null;
            }
    ...
    protected <T> void sendRequest(DefaultHttpClient client, HttpContext httpContext, HttpUriRequest uriRequest, String contentType, AjaxCallBack<T> ajaxCallBack) {
        ...
        HttpHandler<T> task = new HttpHandler<T>(client, httpContext, ajaxCallBack,charset);
        tasks.add(task);
        task.executeOnExecutor(executor, uriRequest);
    }
